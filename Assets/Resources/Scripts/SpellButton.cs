    ...
    public void OnClick()
    {
        ...
        if ((curSpellBtnEnum == SpellBtnEnum.LeftBtn || curSpellBtnEnum == SpellBtnEnum.SelectBtn) && spellData != null) 
        {
            ...
            if (spellData.spellType == SpellData.SpellType.Creature)//크리쳐를 대상으로 하는 경우
            {
                ...
                Bullet spellCreatureBullet = spellCreature.useBullet.GetComponent<Bullet>();
                int spellCreatureBulletDamage = spellCreatureBullet.bulletDamage;
                string rangeType = "<color=#406BFF>원거리</color>";
                if (spellCreature.bulletStartPoint == null) 
                {
                    rangeType = "<color=#406BFF>근접</color>";
                }
                //체력, 공격력, 사거리, 이동속도
                selectManager.selectedSpellInfo.text =
                "<color=#406BFF>체력</color>: " + spellCreature.maxHealth + "\n" +
                "<color=#406BFF>공격력</color>: " + spellCreatureBulletDamage / 3 + '/' + spellCreatureBulletDamage / 2 + '/' + spellCreatureBulletDamage + "(<color=#406BFF>난이도별</color>)" + "\n" +
                "<color=#406BFF>공격 사거리</color>: " + spellCreature.maxRange + "(" + rangeType + ")\n" +
                "<color=#406BFF>기동력</color>: " + spellCreature.nav.speed;
            }
            else//스펠을 대상으로 하는 경우
            {
                ...
                if (spellWeaponBullet.curBulletEffectEnum == Bullet.BulleEffectEnum.Damage)
                {
                    selectManager.selectedSpellInfo.text = "<color=#406BFF>공격력</color>: " +
                            spellWeaponBulletDamage / 3 + "/" + spellWeaponBulletDamage / 2 + "/" + spellWeaponBulletDamage + "(<color=#406BFF>난이도별</color>)" + "\n";
                }
                else
                {
                    selectManager.selectedSpellInfo.text = "<color=#406BFF>치유력</color>: " + spellWeaponBulletDamage + "\n";
                }

                selectManager.selectedSpellInfo.text += "<color=#406BFF>피격 대상</color>: ";
                if (spellWeaponBullet.bulletTarget != null)
                    selectManager.selectedSpellInfo.text += "단일 " + "\n";
                else
                    selectManager.selectedSpellInfo.text += "다중(<color=#406BFF>" + spellWeaponBulletRad + "</color>)\n";
